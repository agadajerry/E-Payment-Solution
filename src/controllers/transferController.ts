...
export const registerUser = async (
  ...
  try {
    ...
    userRegistration.findOne({ email: email }, async (err: any, user: any) => {
      ...
      return  res.status(201).json({ msg: "New User registered successfully...." });
      }
...
export const getBalanceByUserId = (
  ...
  try {
    ...
    balances.findById(req.params.userId, (err: any, user: any) => {
      if (err) {
        return res.json({
          msg: "error message in user account balance fetching" + err,
        });
  ...
  try {
    ...
    transScheme
      ...
      .exec((err: any, userTrans: any) => {
        ...
        if (userTrans) {
          return res.status(201).json(userTrans);
        } else {
  ...
  try {
    ...
    transScheme
      ...
      .exec((err: any, userTrans: any) => {
        ...
        if (userTrans) {
          return res.status(201).json({msg:"Successful", status:"Debit", userTrans});
        } else {
          res.json({ msg: "User account number provided does not exist" });
        }
      });
